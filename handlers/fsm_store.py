...
async def load_name(message: types.Message, state: FSMContext):
    ...
    await message.answer('Укажите коллекцию: ', reply_markup=buttons.cancel_button)
    await FSM_Store.next()


async def load_collection_name(message: types.Message, state: FSMContext):
    async with state.proxy() as data:
        data['collection_product'] = message.text

    await message.answer('Введите инфо: ', reply_markup=buttons.cancel_button)
    await FSM_Store.next()
    ...
    if message.text == 'Да':
        async with state.proxy() as data:
            ...
            await db_main.sql_insert_collection_product(
                collection_product=data['collection_product'],
                product_id=data['product_id']
            )
            await state.finish()
...
def register_store(dp: Dispatcher):
    ...
    dp.register_message_handler(load_collection_name, state=FSM_Store.collection_product)
